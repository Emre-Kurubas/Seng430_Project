...
const ResultsEvaluation = ({ isDarkMode, onNext, onPrev, onGoToStep, trainedModelResult, domain }) => {
    ...
    const getThresholdMetrics = (t, base) => {
        ...
        let sens, spec;
        const bSens = base.sensitivity ?? 0.5;
        const bSpec = base.specificity ?? 0.5;
                    ...
                    <motion.div variants={containerAnim} initial="hidden" animate="show" className="w-full space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                ...
                                >
                                    <CMCell value={results?.tp ?? 0} label="Correctly Caught" sub="True Positives" type="tp" isDarkMode={isDarkMode} delay={0.1} />
                                    <CMCell value={results?.fn ?? 0} label="Missed Risk" sub="False Negatives" type="fn" isDarkMode={isDarkMode} delay={0.2} />
                                    <CMCell value={results?.tn ?? 0} label="Correctly Safe" sub="True Negatives" type="tn" isDarkMode={isDarkMode} delay={0.15} />
                                    <CMCell value={results?.fp ?? 0} label="False Alarms" sub="False Positives" type="fp" isDarkMode={isDarkMode} delay={0.25} />
                                </motion.div>
