...
const SVMViz = React.memo(({ params, isDarkMode, datasetSchema, targetColumn, primaryStr, secondaryStr, dataset }) => {
    ...
    const { fNames, redPts, greenPts, svRedIdx, svGreenIdx, marginScale, svCount } = useMemo(() => {
        ...
        const minSV_D = Math.max(0.015, Math.min(redDists[0]?.d || 0.1, greenDists[0]?.d || 0.1));
    ...
    const draw = useCallback(() => {
        ...
        if (isLinear) {
            ...
            // Mathematically precise Margin Fill matching SVs perfectly
            ctx.save(); ctx.translate(w*0.5,h*0.5); ctx.rotate(-Math.PI/4);
            ctx.fillStyle = COLORS.marginFill;
            // The canvas bounding radius in linear is projected pixel space:
            // Distance d in scaled normalized coords -> physical pixels: exactMarginD * w * sqrt(2)
            const physicalMarginPx = exactMarginD * w * Math.sqrt(2);
            ctx.fillRect(-physicalMarginPx, -h, physicalMarginPx * 2, h*2); 
            ctx.restore();
        } else {
            ...
            ctx.fillStyle = COLORS.marginFill;
            ctx.beginPath(); 
            // Ellipse margins drawn expanding exactly at SV radius
            const rx = w * 0.25 * (1 + exactMarginD);
            const ry = h * 0.25 * (1 + exactMarginD);
            ctx.ellipse(w*0.5, h*0.5, rx, ry, Math.PI/4, 0, Math.PI*2);
            ctx.ellipse(w*0.5, h*0.5, w * 0.25 * Math.max(0.01, 1 - exactMarginD), h * 0.25 * Math.max(0.01, 1 - exactMarginD), Math.PI/4, 0, Math.PI*2, true);
            ctx.fill();
        }
    ...
    const treeData = useMemo(() => {
        ...
        const traverse = (n, depth) => {
            ...
            if (n.splitColumn !== undefined && n.splitValue !== undefined) {
                 ...
                 if (c1 >= c0) {
                     label = `High Risk of ${classLabel1}\n(${pct}% Conf)`;
                     color = COLORS.red;
                 } else {
                     label = `Low Risk\n(${100 - pct}% Conf)`;
                     color = COLORS.green;
    ...
    const features = useMemo(() => {
        ...
        const processed = validCols.map(fName => {
            ...
            const impactText = impactVal > 15
                ? `Strong risk ${direction === 'increase' ? 'ACCELERATOR' : 'REDUCER'} finding: ${sign}${impactVal}%`
                : impactVal > 5
                    ? `${direction === 'increase' ? 'Elevates' : 'Lowers'} relative risk trajectory by: ${sign}${impactVal}%`
                    : `Marginal clinical ${direction === 'increase' ? 'aggravation' : 'mitigation'} detected.`;
                    ...
                    return (
                        ...
                        >
                            <div className="flex justify-between items-center mb-2">
                                <span className={'text-[12px] font-semibold ' + (isDarkMode ? 'text-slate-200' : 'text-slate-700')}>Sample condition: {f.name} {" > "} mean</span>
