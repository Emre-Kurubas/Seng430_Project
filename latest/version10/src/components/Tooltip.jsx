...
const Tooltip = ({ children, content, isDarkMode, position = 'top', noUnderline, className = '' }) => {
    ...
    return (
        <>
            <span
                ...
                onFocus={show}
                onBlur={hide}
                className={`cursor-help ${!noUnderline ? 'underline decoration-dotted decoration-slate-400 underline-offset-2' : ''} ${className}`}
            >
                {children}
            </span>
            {visible && createPortal(
                <div
                    ref={tooltipRef}
                    role="tooltip"
                    className={`fixed z-[9999] max-w-[260px] px-3 py-2 rounded-lg text-xs leading-relaxed shadow-sm border pointer-events-none
                        ${isDarkMode
