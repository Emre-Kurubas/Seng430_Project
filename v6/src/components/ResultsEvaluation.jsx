...
const MetricCard = React.memo(({ label, value, desc, interpretation, concern, isDarkMode, star, delay = 0 }) => {
    ...
    return (
        <motion.div
            ...
            className={`p-4 rounded-xl border transition-all duration-300 group relative
                ${isDarkMode
        ...
        >
            ...
            <div className={`text-2xl font-black mb-1 ${color}`}>{animatedPct}%</div>
...
const ResultsEvaluation = ({ isDarkMode, onNext, onPrev, onGoToStep, trainedModelResult, domain }) => {
    ...
    return (
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="w-full space-y-6 pb-20">
            ...
            <motion.div variants={itemAnim} className="mb-2">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3">
                    <div className="flex items-center gap-3">
                        <span className={`text-xs font-medium px-2 py-1 rounded-md ${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-100'}`}>
                            Step 5 of 7
                        </span>
                    </div>
                ...
                <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    Evaluate the model predictions on the test patients. We look beyond accuracy to sensitivity (catching real cases) and specificity (avoiding false alarms).
                </p>
            </motion.div>
            ...
            {/* ── Loading State ── */}
            {loading && (
                <div className={`rounded-xl border p-16 flex flex-col items-center justify-center gap-4 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            {results && (
                <>
                    ...
                    <motion.div variants={itemAnim} className={`p-6 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {/* ROC Curve */}
                        <motion.div variants={itemAnim} className={`p-6 rounded-xl border flex flex-col ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                            <div className="flex items-center gap-2 mb-1">
                    ...
                    {/* ── Threshold Explorer ── */}
                    {results && (
                        <motion.div variants={itemAnim} className={`p-6 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                            <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                                {[
                                    ...
                                    return (
                                        <div key={m.label} className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    {/* ── Retrain Button ── */}
                    {results && (
                        <motion.div variants={itemAnim} className={`p-5 rounded-xl border flex flex-col sm:flex-row items-center justify-between gap-4 ${isDarkMode ? 'bg-indigo-900/15 border-indigo-500/25' : 'bg-indigo-50 border-indigo-200'}`}>
                            <div>
                                <h4 className={`text-sm font-bold mb-1 ${isDarkMode ? 'text-indigo-300' : 'text-indigo-800'}`}>
                                    🔄 Want to try a different model or adjust parameters?
                                </h4>
                                <p className={`text-xs ${isDarkMode ? 'text-indigo-400/70' : 'text-indigo-600/70'}`}>
                                    Go back to Step 4 to retrain with different settings, then use "+ Compare Model" to compare results side by side.
                                </p>
                            </div>
                            <button
                                onClick={() => onGoToStep && onGoToStep(4)}
                                className={`shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all  shadow-md
                                    ${isDarkMode
                    ...
                    <motion.div variants={itemAnim} className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                    ...
                    {/* ── Navigation ── */}
                    <motion.div variants={itemAnim} className="flex justify-between items-center pt-4">
                        <button
                            onClick={onPrev}
