...
import { calculateGlobalFeatureImportance, calculateLocalSHAP } from '../utils/mlEngine';
...
const RiskDriversCard = ({ featureImportance, isDarkMode }) => {
    return (
        <div id="s6-global-risk" className={`${isDarkMode ? 'bg-[#1c1c1e] border-[#2c2c2e]' : 'bg-white border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]'} rounded-[40px] p-8 md:p-10 w-full flex flex-col border relative overflow-hidden transition-colors h-full`}>
            {/* Header */}
...
const PatientSelectCard = ({ patients, selectedPatient, onSelect, onExplain, explaining, loadingFeatures, isDarkMode, primaryStr }) => (
    <div id="s6-patient-select" className={`${isDarkMode ? 'bg-[#1c1c1e] border-[#2c2c2e]' : 'bg-white border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]'} rounded-[36px] p-7 md:p-8 flex flex-col justify-between border flex-1 min-h-[220px] transition-colors`}>
        ...
        <div className="relative mb-3">
            <select value={selectedPatient} onChange={e => { onSelect(Number(e.target.value)); }}
                className={`w-full py-3 pl-4 pr-10 rounded-2xl border text-[13px] font-medium outline-none appearance-none cursor-pointer transition-all duration-200 focus:ring-2 focus:ring-offset-0 ${
                    isDarkMode 
                        ? 'bg-white/[0.04] border-white/[0.08] text-slate-200 hover:bg-white/[0.06] focus:border-white/[0.15] focus:ring-white/[0.08]' 
                        : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100 focus:border-slate-300 focus:ring-slate-200/50'
                }`}>
                {patients.map(p => <option key={p.id} value={p.id} className={isDarkMode ? 'bg-[#27272a] text-slate-200' : 'bg-white text-slate-700'}>{p.label}</option>)}
            </select>
            <ChevronDown className={`absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
        </div>
...
const ClinicalValidationCard = ({ senseCheck, top1, isDarkMode }) => {
    const pct = top1 ? Math.round(top1.importance * 100) : 0;
    const circumference = 2 * Math.PI * 38;
    const strokeDash = (pct / 100) * circumference;

    return (
        <div id="s6-validation" className={`${isDarkMode ? 'bg-[#1c1c1e] border-[#2c2c2e]' : 'bg-white border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]'} rounded-[36px] p-7 md:p-8 flex flex-col justify-between border flex-1 min-h-[220px] transition-colors`}>
            ...
            {/* Centered ring + percentage */}
            <div className="flex-1 flex flex-col items-center justify-center py-3">
                <div className="relative w-[100px] h-[100px] flex items-center justify-center">
                    <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full -rotate-90">
                        <circle cx="50" cy="50" r="38" fill="none" stroke={isDarkMode ? '#27272a' : '#f1f5f9'} strokeWidth="6" />
                        <motion.circle cx="50" cy="50" r="38" fill="none" stroke="#c8f560" strokeWidth="6" strokeLinecap="round"
                            initial={{ strokeDasharray: `0 ${circumference}` }}
                            animate={{ strokeDasharray: `${strokeDash} ${circumference}` }}
                            transition={{ duration: 1.5, ease: 'easeOut' }}
                            style={{ filter: 'drop-shadow(0 0 6px rgba(200,245,96,0.4))' }}
                        />
                    </svg>
                    <span className={`text-[32px] font-bold tracking-tight leading-none ${isDarkMode ? 'text-white' : 'text-[#111111]'}`}>
            ...
            <div className="text-center">
                <div className={`text-[11px] font-bold uppercase tracking-widest mb-1 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Top Driver</div>
                <div className={`text-[17px] font-bold tracking-tight leading-tight ${isDarkMode ? 'text-white' : 'text-[#111111]'}`}>
                    {top1 ? formatFeatureName(top1.label) : 'N/A'}
                </div>
            </div>
        </div>
    );
...
const SHAPAnalysisCard = ({ patientData, explained, explaining, patients, selectedPatient, primaryStr, isDarkMode, whatIf }) => (
    <div id="s6-shap" className={`${isDarkMode ? 'bg-[#161618] border border-white/5 text-white' : 'bg-white border text-slate-800 shadow-[0_4px_24px_rgba(0,0,0,0.02)]'} rounded-[36px] p-7 md:p-8 flex flex-col relative overflow-hidden transition-colors w-full min-h-[460px] h-full`}>
...
const Explainability = ({ isDarkMode, onNext, onPrev, domain, dataset, datasetSchema, targetColumn, trainedModelResult }) => {
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            let importance = [];
            if (trainedModelResult?.predictFn && trainedModelResult?.featureCols?.length > 0) {
                const realScores = calculateGlobalFeatureImportance(
                    trainedModelResult.predictFn,
                    trainedModelResult.XTest,
                    trainedModelResult.yTest,
                    trainedModelResult.featureCols
                );
                
                // Map the calculated importance back to our visual features list
                importance = activeFeatures.map(f => {
                    const match = realScores.find(rs => rs.id === f.id);
                    return { ...f, importance: match ? match.importance : 0.01 };
                }).sort((a, b) => b.importance - a.importance).slice(0, 8);
            } else {
                importance = generateFeatureImportance(activeFeatures);
            }
    ...
    const handleExplain = () => {
        ...
        setTimeout(() => { 
            ...
            if (trainedModelResult?.predictFn && trainedModelResult?.XTrain?.length > 0 && patient?.rowData) {
                // Extract just the feature columns from the patient row to match what model expects
                const featureCols = trainedModelResult.featureCols;
                const patientRowArray = featureCols.map(col => {
                    const val = Number(patient.rowData[col]);
                    return isNaN(val) ? 0 : val;
                });

                const shap = calculateLocalSHAP(
                    trainedModelResult.predictFn,
                    patientRowArray,
                    trainedModelResult.XTrain,
                    featureCols
                );

                // Map back to expected visualization structure
                const contributions = activeFeatures.map(f => {
                    const match = shap.contributions.find(c => c.id === f.id);
                    const magnitude = match ? match.contribution : 0;
                    const val = patient.rowData[f.id];
                    let dispVal = 'N/A';
                    if (val !== undefined && val !== null) dispVal = typeof val === 'number' ? (val % 1 === 0 ? String(val) : val.toFixed(2)) : String(val).slice(0, 20);
                    return { 
                ...
                data = { contributions, riskScore: shap.patientRisk };
            } else {
                data = generatePatientContributions(activeFeatures, patient);
            }
    ...
    return (
        ...
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="relative w-full pb-44 max-w-6xl mx-auto">
            ...
            <StepTour
                ...
                steps={[
                    {
                        targetId: 's6-global-risk',
                        position: 'right',
                        pose: 'point',
                        title: 'Global Feature Importance',
                        body: 'This "Risk Drivers" panel shows Global Explainability — it tells us what the model looks at most across the entire dataset. The overlapping bubbles and bars show the percentage impact of each feature. This helps us verify that the model learned medically sound rules, not just statistical noise.',
                        sub: 'For example, if Age is the biggest bubble, the model relies heavily on Age to make its predictions for everyone.',
                    },
                    {
                        targetId: 's6-patient-select',
                        position: 'below',
                        pose: 'wave',
                        title: 'Patient Selection',
                        body: 'Global rules are great, but how does the model treat a specific individual? Here, you can select a single mock patient from the dataset. Click "Explain" to calculate the Local Explainability for that exact person.',
                        sub: 'In a real clinical dashboard, this would be the patient currently sitting in front of you.',
                    },
                    {
                        targetId: 's6-validation',
                        position: 'below',
                        pose: 'point',
                        title: 'Clinical Validation Check',
                        body: 'This panel acts as a quick "sanity check". It compares the top driver found by the AI with established medical literature for your chosen domain. It ensures that the model\'s logic aligns with human medical knowledge.',
                        sub: 'An AI must be trustworthy. If it uses a medically irrelevant feature to predict risk, it fails this clinical validation.',
                    },
                    {
                        targetId: 's6-shap',
                        position: 'left',
                        pose: 'wave',
                        title: 'Local SHAP Analysis',
                        body: 'Once you click "Explain", this area visualises the exact reasons for the selected patient\'s risk score using SHAP values. Green bars are "Safe Factors" pushing the risk down, while Purple bars are "Risk Factors" pushing it up. Hover over the bars to see exactly how much each metric contributed.',
                        sub: 'This is the most crucial part of Explainable AI (XAI). It answers the doctor\'s ultimate question: "Why did you make this prediction for MY patient?"',
                    },
                ]}
            />
        </>
