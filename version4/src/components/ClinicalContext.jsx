            ...
            <motion.div variants={item} className={`flex flex-col md:flex-row justify-between items-start gap-6 backdrop-blur-md p-5 sm:p-8 rounded-3xl border shadow-xl transition-all duration-500 ${isDarkMode ? 'bg-white/5 border-white/10 shadow-black/20' : 'bg-white/80 border-slate-200 shadow-slate-200/50'}`}>
                <div className="space-y-4 sm:space-y-6 max-w-4xl relative">
                    <div className="flex flex-wrap items-center gap-3">
                        <span className={`backdrop-blur border px-3 sm:px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase shadow-sm flex items-center gap-2 ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-white border-slate-200 text-slate-700'}`} style={{ color: primaryStr, borderColor: `${primaryStr}40`, backgroundColor: `${primaryStr}10` }}>
                            <span className="relative flex h-2 w-2">
                                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75`} style={{ backgroundColor: primaryStr }}></span>
                                <span className={`relative inline-flex rounded-full h-2 w-2`} style={{ backgroundColor: primaryStr }}></span>
                            </span>
                            Step 1 of 7
                        </span>
                    ...
                    <h1 className={`text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight leading-tight ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}>
                        Clinical Context & <span style={{ backgroundImage: `linear-gradient(to right, ${primaryStr}, ${secondaryStr})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Problem Definition</span>
                    </h1>
                    ...
                    <p className={`text-base sm:text-lg md:text-xl leading-relaxed max-w-3xl font-light ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                        Before we look at any data, we define the clinical problem. In <strong className={`font-semibold border-b-2 ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`} style={{ borderBottomColor: primaryStr }}>{domain.name}</strong>, we want to predict <strong>{domain.description}</strong>.
                    </p>
                </div>
                ...
                <motion.button
                    ...
                    className={`w-full md:w-auto pl-6 sm:pl-8 pr-4 sm:pr-6 py-3 sm:py-4 rounded-2xl font-bold transition-all flex items-center justify-center space-x-3 group text-base sm:text-lg shrink-0 border md:min-w-[200px] text-white border-white/10`}
                >
                    <span>Next Step</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
                ...
                <motion.div variants={item} className="lg:col-span-5 flex flex-col h-full">
                    <div className={`backdrop-blur-xl rounded-[2rem] p-5 sm:p-8 shadow-xl border relative overflow-hidden h-full transform transition-all duration-500 ${isDarkMode ? 'bg-slate-800/40 shadow-black/20 border-white/5 hover:shadow-2xl hover:shadow-indigo-500/10' : 'bg-white/60 shadow-slate-200/50 border-slate-200 hover:shadow-xl hover:shadow-indigo-100'}`}>
                        ...
                        <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -z-10 translate-x-1/2 -translate-y-1/2" style={{ backgroundColor: `${primaryStr}1A` }}></div>
                        <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full blur-3xl -z-10 -translate-x-1/2 translate-y-1/2" style={{ backgroundColor: `${secondaryStr}1A` }}></div>
                        ...
                        <div className="relative z-10 space-y-8">
                            ...
                            <div>
                                ...
                                <div className="group flex items-center space-x-3 transition-colors">
                                    <div className="p-3 rounded-2xl transition-colors border" style={{ backgroundColor: `${primaryStr}15`, borderColor: `${primaryStr}30` }}>
                                        <DomainIcon className="w-8 h-8 drop-shadow-sm" style={{ color: primaryStr }} />
                                    </div>
                                    <span className="text-2xl md:text-3xl font-black tracking-tight transition-colors" style={{ color: primaryStr }}>{domain.name}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <div className={`border p-6 rounded-2xl font-medium text-lg leading-relaxed shadow-sm backdrop-blur-sm relative`} style={{ backgroundColor: `${primaryStr}0A`, borderColor: `${primaryStr}20`, color: isDarkMode ? '#e2e8f0' : '#334155' }}>
                                        <span className="absolute -top-3 left-6 text-6xl font-serif leading-none select-none" style={{ color: `${primaryStr}30` }}>"</span>
