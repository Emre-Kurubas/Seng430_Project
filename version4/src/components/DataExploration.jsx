...
/**
 * @component ClassBalanceChart
 * @description Displays a horizontal bar chart showing the distribution of the target class.
 * Mentions if the data is heavily imbalanced (>60% majority).
 * @param {Object} props
 * @param {Object} props.classBalance - Record of class labels to counts
 * @param {boolean} props.isLoading - Native loading state
 ...
 * @description Step 2: Allows users to upload a custom CSV dataset or load the default domain dataset.
 * Parses the CSV, calculates missing data, and requires Column Mapper validation before proceeding.
 * @param {Object} props
 * @param {boolean} props.isDarkMode - Dark mode styling flag
 * @param {Function} props.onNext - Callback to advance to Step 3
 * @param {Function} props.onPrev - Callback to return to Step 1
 * @param {Object} props.domain - Currently selected specialty domain
 * @param {Function} props.onPatientCountChange - Callback to report total rows loaded
 * @param {Array<Object>} props.dataset - Shared state: array of data rows
 * @param {Function} props.setDataset - Shared state setter for data rows
 * @param {Array<Object>} props.datasetSchema - Shared state: validated schema from Column Mapper
 * @param {Function} props.setDatasetSchema - Shared state setter for schema
 * @param {string} props.targetColumn - Name of the predicted target column
 * @param {Function} props.setTargetColumn - Setter for the target column
 */
        ...
        <div className="w-full space-y-6 animate-in fade-in duration-500">
            ...
            <div className={'p-4 sm:p-6 rounded-2xl border shadow-sm step-accent ' + (isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200')}>
                <div className="flex items-center justify-between mb-4">
                    <span className={'text-xs font-bold px-2 py-1 rounded-full border'} style={{ backgroundColor: isDarkMode ? `${primaryStr}20` : `${primaryStr}1A`, color: primaryStr, borderColor: `${primaryStr}33` }}>
                        STEP 2 OF 7
                    </span>
                    <button
                        onClick={handleNextClick}
                        className={'flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all border ' + (isMapped
                            ? 'text-white shadow-md hover:shadow-lg hover:scale-105 active:scale-95 border-transparent'
                            : isDarkMode ? 'bg-slate-700 text-slate-400 hover:bg-slate-600 border-slate-600' : 'bg-slate-200 text-slate-400 hover:bg-slate-300 border-slate-300')}
                        style={isMapped ? { backgroundColor: secondaryStr, boxShadow: `0 4px 14px 0 ${secondaryStr}40` } : {}}
                    >
                            ...
                            <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                                <label
                                    className={'relative overflow-hidden border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center transition-colors cursor-pointer ' + (isDarkMode
                                        ? `border-slate-700 hover:bg-slate-800/50 ${isDragging ? 'bg-slate-800/50 border-transparent' : 'bg-slate-800/30'}`
                                        : `border-slate-300 hover:bg-slate-100 ${isDragging ? 'bg-slate-100 border-transparent' : 'bg-slate-50'}`)}
