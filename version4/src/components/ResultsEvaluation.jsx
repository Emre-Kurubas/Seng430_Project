...
// We no longer simulate results randomly; we receive results from the ML engine.
...
const ROCCurve = React.memo(({ auc, isDarkMode, primaryStr }) => {
    ...
    const curveColor = primaryStr || '#6366F1';
...
const ResultsEvaluation = ({ isDarkMode, onNext, onPrev, trainedModelResult, domain }) => {
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            if (trainedModelResult) {
                setResults(trainedModelResult);
            }
            setLoading(false);
        }, 400);
    ...
    const metrics = useMemo(() => results ? [
        { label: 'Accuracy', value: results.accuracy, concern: 0.60, desc: 'Overall correct predictions.',
          interpretation: results.accuracy >= 0.80 ? 'Strong: The AI correctly classifies the majority of patients.' : results.accuracy >= 0.60 ? 'Moderate: The AI is correct more often than not, but has room for improvement.' : 'Weak: The AI is making too many errors overall — consider a different model or more data.',
          delay: 0.05 },
        { label: 'Sensitivity', value: results.sensitivity, concern: 0.50, desc: 'Catch rate for real cases.',
          interpretation: results.sensitivity >= 0.80 ? 'Excellent: The AI catches most patients who truly have the condition.' : results.sensitivity >= 0.60 ? 'Acceptable: Some real cases are being missed — monitor false negatives carefully.' : 'Dangerous: The AI is missing too many sick patients. This model is not safe for screening.',
          delay: 0.10, star: true },
        { label: 'Specificity', value: results.specificity, concern: 0.60, desc: 'Efficiency in avoiding false alarms.',
          interpretation: results.specificity >= 0.80 ? 'Efficient: Few healthy patients are unnecessarily flagged.' : results.specificity >= 0.60 ? 'Fair: Some false alarms, but manageable with clinical resources.' : 'Poor: Too many false alarms — staff will waste time on patients who are actually safe.',
          delay: 0.15 },
        { label: 'Precision', value: results.precision, concern: 0.60, desc: 'Confidence in flagged cases.',
          interpretation: results.precision >= 0.80 ? 'Reliable: When the AI flags a patient, it is usually correct.' : results.precision >= 0.60 ? 'Moderate: Some flagged patients turn out to be fine — expect some unnecessary follow-ups.' : 'Low: Most flagged patients are false alarms — clinical trust in the model is weakened.',
          delay: 0.20 },
        { label: 'F1 Score', value: results.f1Score, concern: 0.60, desc: 'Balanced performance measure.',
          interpretation: results.f1Score >= 0.80 ? 'Well-balanced: The model handles both missed cases and false alarms effectively.' : results.f1Score >= 0.60 ? 'Acceptable balance, but improving Sensitivity or Precision could help.' : 'Imbalanced: The model struggles to juggle catching cases and avoiding false alarms.',
          delay: 0.25 },
        { label: 'AUC-ROC', value: results.auc, concern: 0.60, desc: 'Discrimination ability.',
          interpretation: results.auc >= 0.80 ? 'Strong separation: The model reliably distinguishes high-risk from low-risk patients.' : results.auc >= 0.60 ? 'Moderate: The model can separate groups but overlaps exist — useful with caution.' : 'Poor separation: The model cannot reliably tell patient groups apart — close to random chance.',
          delay: 0.30 },
    ] : [], [results]);
        ...
        <div className="w-full space-y-6 pb-20">
            ...
            <div className={`p-4 sm:p-6 rounded-2xl border step-accent ${isDarkMode ? 'glass-depth-1' : 'glass-depth-light-1'}`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div className="flex items-center gap-3">
                        <span className={`text-xs font-bold px-2 py-1 rounded-full`} style={{ backgroundColor: `${primaryStr}20`, color: primaryStr }}>
                            STEP 5 OF 7
                        </span>
                    </div>
            ...
            {loading && (
                <div className={`rounded-2xl border p-16 flex flex-col items-center justify-center gap-4 ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
                    <div className={`w-12 h-12 border-4 rounded-full animate-spin`} style={{ borderTopColor: primaryStr, borderColor: `${primaryStr}4D` }} />
                        ...
                        <div className={`p-6 rounded-2xl border flex flex-col card-inner-shine ${isDarkMode ? 'glass-depth-1' : 'glass-depth-light-1'}`}>
                            ...
                            <div className={`mt-3 flex gap-4 text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                                <div className="flex items-center gap-1.5">
                                    <div className="w-5 h-0.5" style={{ backgroundColor: primaryStr }} />
                        ...
                        <div className={`p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                            ...
                            <div className="mb-6">
                                ...
                                <input
                                    ...
                                    style={{ accentColor: primaryStr, background: `linear-gradient(to right, ${primaryStr} 0%, ${primaryStr} ${((threshold - 10) / 80) * 100}%, ${isDarkMode ? '#334155' : '#e2e8f0'} ${((threshold - 10) / 80) * 100}%, ${isDarkMode ? '#334155' : '#e2e8f0'} 100%)` }}
                                />
