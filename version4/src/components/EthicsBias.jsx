...
const generateSubgroups = (trainedModelResult) => {
    const overall = trainedModelResult ? {
        acc: trainedModelResult.accuracy,
        sens: trainedModelResult.sensitivity,
        spec: trainedModelResult.specificity
    } : { acc: rand(0.72, 0.82), sens: rand(0.65, 0.75), spec: rand(0.72, 0.85) };
    ...
    return {
        overall,
        rows: [
            { group: 'Male', acc: Math.min(baseAcc + rand(0.01, 0.05), 1), sens: Math.min(baseSens + rand(0.01, 0.04), 1), spec: Math.min(spec + rand(0.01, 0.05), 1), fairness: 'reference' },
            { group: 'Female', acc: Math.max(baseAcc - rand(0.02, 0.08), 0.01), sens: Math.max(baseSens - rand(0.12, 0.25), 0.01), spec: spec, fairness: null },
            { group: 'Age 18–60', acc: baseAcc, sens: baseSens, spec: spec, fairness: null },
            { group: 'Age 61–75', acc: Math.max(baseAcc - rand(0.01, 0.05), 0.01), sens: Math.max(baseSens - rand(0.05, 0.15), 0.01), spec: spec, fairness: null },
            { group: 'Age 76+', acc: Math.max(baseAcc - rand(0.05, 0.1), 0.01), sens: Math.max(baseSens - rand(0.15, 0.30), 0.01), spec: spec, fairness: null },
        ],
    };
};
...
// ─── PDF Certificate (server-side first, client-side fallback) ────────────────
const downloadCertificate = async (checked, subgroups, domain, trainedModelResult, modelName) => {
    const total = CHECKLIST_ITEMS.length;
    const done = checked.length;
    const overall = subgroups?.overall;
    const dateStr = new Date().toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' });
    const mName = modelName || 'Unknown Model';

    // Try server-side certificate generation first
    try {
        ...
        const response = await fetch(`${API_BASE}/api/generate-certificate`, {
            ...
            body: JSON.stringify({
                domain: domain?.name || 'Cardiology',
                modelName: mName,
                metrics: trainedModelResult ? {
                    accuracy: trainedModelResult.accuracy,
                    sensitivity: trainedModelResult.sensitivity,
                    specificity: trainedModelResult.specificity,
                    precision: trainedModelResult.precision,
                    f1Score: trainedModelResult.f1Score,
                    auc: trainedModelResult.auc,
                } : null,
                biasFindings: (subgroups?.rows || []).map(r => ({ group: r.group, sens: r.sens })),
                checklist: {
                    items: CHECKLIST_ITEMS.map(i => i.label),
                    checked: CHECKLIST_ITEMS.filter(i => checked.includes(i.id)).map(i => i.label),
                },
            }),
        });
        ...
        console.warn('[Certificate] Server-side generation failed, falling back to client-side:', e.message);
    }
...
<div class="cert">
  ...
  <table>
    ...
    <tr><th>Model Used</th><td>${mName}</td></tr>
  ...
  <table>
    ...
    <tr><td>Precision</td><td>${trainedModelResult?.precision ? Math.round(trainedModelResult.precision * 100) + '%' : 'N/A'}</td><td><span class="badge ${(trainedModelResult?.precision || 0) >= 0.60 ? 'green' : 'amber'}">${(trainedModelResult?.precision || 0) >= 0.60 ? 'Acceptable' : 'Review'}</span></td></tr>
    <tr><td>F1 Score</td><td>${trainedModelResult?.f1Score ? Math.round(trainedModelResult.f1Score * 100) + '%' : 'N/A'}</td><td><span class="badge ${(trainedModelResult?.f1Score || 0) >= 0.60 ? 'green' : 'amber'}">${(trainedModelResult?.f1Score || 0) >= 0.60 ? 'Acceptable' : 'Review'}</span></td></tr>
    <tr><td>AUC-ROC</td><td>${trainedModelResult?.auc ? Math.round(trainedModelResult.auc * 100) + '%' : 'N/A'}</td><td><span class="badge ${(trainedModelResult?.auc || 0) >= 0.65 ? 'green' : 'amber'}">${(trainedModelResult?.auc || 0) >= 0.65 ? 'Acceptable' : 'Review'}</span></td></tr>
  </table>
...
// ─── Main Component ───────────────────────────────────────────────────────────
const EthicsBias = ({ isDarkMode, onPrev, domain, trainedModelResult, modelName }) => {
        ...
        <div className="w-full space-y-6 pb-20">
            ...
            <div className={`p-4 sm:p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
                    ...
                    <button
                        onClick={() => downloadCertificate(checkedItems, subgroups, domain, trainedModelResult, trainedModelResult?.modelName)}
                    ...
                    <div className={`rounded-2xl border overflow-hidden ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        ...
                        {loading ? (
                            <div className="p-8 flex items-center justify-center gap-3">
                                <div className={`w-8 h-8 border-4 border-t-[color:var(--theme-primary)] rounded-full animate-spin`} style={{ '--theme-primary': primaryStr, borderTopColor: primaryStr, borderColor: `${primaryStr}4D` }} />
                    ...
                    <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm h-full flex flex-col`}>
                        ...
                        <div className="mb-4">
                            <div className="flex justify-between text-[11px] mb-1">
                                ...
                                <span className={`font-bold ${allChecked ? 'text-emerald-500' : ''}`} style={!allChecked ? { color: primaryStr } : {}}>
                            ...
                            <div className={`h-2 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-700' : 'bg-slate-100'}`}>
                                <motion.div
                                    className={`h-full rounded-full ${allChecked ? 'bg-emerald-500' : ''}`}
                                    style={!allChecked ? { backgroundColor: primaryStr } : {}}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <>
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <RepBar label="Male patients" trainingPct={trainingMalePct} hospitalPct={hospitalMalePct} isDarkMode={isDarkMode} delay={0.05} primaryStr={primaryStr} />
                                </div>
                                <div>
                                    ...
                                    <RepBar label="Female patients" trainingPct={trainingFemalePct} hospitalPct={hospitalFemalePct} isDarkMode={isDarkMode} delay={0.15} primaryStr={primaryStr} />
                                </div>
                            </div>

                            {/* Legend */}
                            <div className={`flex gap-4 mt-4 text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm" style={{ backgroundColor: primaryStr }} /> Training data</div>
                ...
                <div className={`p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                    <div className="flex items-center gap-2 mb-1">
                        <BookOpen className={`w-4 h-4`} style={{ color: primaryStr }} />
                        <h3 className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
