...
const SubgroupRow = React.memo(({ group, acc, sens, spec, overall, isDarkMode, delay, isRef }) => {
    ...
    return (
        <motion.tr initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.35, delay }}
            className={`border-b transition-colors ${isDarkMode ? 'border-slate-700 hover:bg-slate-800' : 'border-slate-100 hover:bg-slate-50'} ${isRef ? (isDarkMode ? 'bg-slate-800/20' : 'bg-slate-50/60') : ''}`}>
...
const ChecklistItem = React.memo(({ item, checked, onToggle, isDarkMode, delay }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay }}
            className={`rounded-xl border transition-all ${checked ? (isDarkMode ? 'bg-emerald-900/15 border-emerald-500/25' : 'bg-emerald-50/80 border-emerald-200') : (isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200')}`}>
...
const EthicsBias = ({ isDarkMode, onPrev, domain, trainedModelResult }) => {
    ...
    return (
        <div className="w-full space-y-6 pb-20">
            ...
            <div className={`p-5 rounded-xl border-l-4 flex gap-4 ${isDarkMode ? 'bg-indigo-900/10 border-indigo-500/50 text-indigo-100' : 'bg-indigo-50 border-indigo-500/50 text-indigo-900 shadow-sm'}`}>
            ...
            <div className="mb-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-3">
                    ...
                    <button onClick={handleGenerateCert} disabled={isGenerating} className={`flex items-center gap-2 px-5 py-2 rounded-md text-sm font-semibold text-white transition-colors ${isGenerating ? 'bg-slate-400' : 'bg-emerald-600 hover:bg-emerald-500'}`}>
                        {isGenerating ? 'Generating...' : <><Download className="w-4 h-4" /> Certificate</>}
                    </button>
                </div>
                <h2 className={`text-2xl font-bold mb-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Ethics & Bias</h2>
                <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Audit fairness and regulatory compliance before deployment.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5">
                    <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            <div className="flex justify-between pt-6">
                ...
                <div className="flex gap-4">
                    <button onClick={handleGenerateCert} disabled={isGenerating} className={`px-8 py-3 rounded-xl font-bold flex items-center gap-2 text-white shadow-sm transition-all  relative overflow-hidden ${isGenerating ? 'bg-slate-700' : 'bg-emerald-600 hover:bg-emerald-500'}`}>
                        {isGenerating ? (
