...
const generatePatients = (features, dataset, targetColumn) => {
    ...
    return selected.map((row, i) => {
        ...
        const targetDisplay = targetColumn
            .replace(/_/g, ' ')
        ...
        return {
            ...
            riskLabel,
            vitals: vitalsArr,
            label: `Patient ID #${Math.floor(Math.random() * 8999) + 1000} · ${targetDisplay}: ${riskLabel}`,
            rowData: row
        };
    });
};
...
const Explainability = ({ isDarkMode, onNext, onPrev, domain, dataset, datasetSchema, targetColumn }) => {
    ...
    const CLINICAL_DESC = {
        ...
        'diabetes': 'Diagnosed diabetes mellitus',
        'anaemia': 'Anaemia status (Yes / No)',
        'anemia': 'Anaemia status (Yes / No)',
        'high_blood_pressure': 'Hypertension diagnosis',
        'ejection_fraction': 'How well the heart pumps blood (%)',
        'creatinine_phosphokinase': 'Heart muscle enzyme level (mcg/L)',
        'serum_creatinine': 'Kidney function marker (mg/dL)',
        'serum_sodium': 'Electrolyte balance indicator (mEq/L)',
        ...
        if (datasetSchema && datasetSchema.length > 0) {
            return datasetSchema
                ...
                .map(col => {
                    ...
                    return {
                        id: col.name,
                        label: toReadable(col.name),
                        clinical: CLINICAL_DESC[colLower] || CLINICAL_DESC[col.name] || 
                            (col.role === 'Category' ? 'Categorical variable' : 'Numeric measurement')
                    };
                });
        }
        ...
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="w-full space-y-6 pb-20">
            ...
            <motion.div variants={itemAnim} className="mb-2">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3">
                    <span className={`text-xs font-medium px-2 py-1 rounded-md w-fit ${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-100'}`}>
                        Step 6 of 7
                    </span>
                    <button
                        onClick={onNext}
                ...
                <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    See which patient measurements were most important, and why a specific patient was flagged as high risk.
                </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5 space-y-4">
                    ...
                    <motion.div variants={itemAnim} className={`p-6 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        ...
                        <button
                            ...
                            className={`w-full py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 text-white shadow-sm hover:scale-[1.01] active:scale-95
                        ...
                        {patients[selectedPatient] && (
                            <motion.div
                                ...
                                className={`mt-4 p-4 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                            >
                ...
                <div className="lg:col-span-7 space-y-4">
                    ...
                    <motion.div variants={itemAnim} className={`p-6 rounded-xl border min-h-[240px] flex flex-col ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center gap-2 mb-1">
                    ...
                    <AnimatePresence>
                        {!loadingFeatures && (
                            ...
                            >
                                ...
                                <div className={`p-4 rounded-xl border flex items-start gap-3 text-[12px] leading-relaxed
                                ...
                                {explained && patientData && (
                                    <motion.div
                                        ...
                                        className={`p-4 rounded-xl border flex items-start gap-3 text-[12px] leading-relaxed
