...
class VizErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <div className={`p-8 rounded-xl border text-center ${this.props.isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
...
const ModelSelection = ({ isDarkMode, onNext, onPrev, dataset, datasetSchema, targetColumn, setTrainedModelResult, domain, comparisonList, setComparisonList }) => {
    ...
    if (isInitialLoading) {
        return (
            <div className={`w-full flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in duration-500 rounded-xl border ${isDarkMode ? 'bg-slate-800/20 border-slate-700/50' : 'bg-slate-100 border-slate-200'}`}>
    ...
    return (
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="w-full space-y-6 pb-20">
            {/* Header */}
            <motion.div variants={itemAnim} className="mb-2">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3">
                    <div className="flex items-center justify-between w-full md:w-auto">
                        <span className={`text-xs font-medium px-2 py-1 rounded-md mr-3 ${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-100'}`}>
                            Step 4 of 7
                        </span>
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        <button
                            onClick={onPrev}
                            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-semibold transition-colors ${isDarkMode
                        ...
                        <button
                            onClick={onNext}
                            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-semibold transition-colors text-white`}
                ...
                <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    Choose an algorithm, adjust settings, and train it on your patient data. Compare their performance.
                </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <motion.div variants={itemAnim} className="lg:col-span-4 space-y-6">
                    ...
                    <div className="relative group">
                        <div className={`flex overflow-x-auto pb-2 mb-2 scrollbar-hide gap-2 p-1.5 rounded-xl ${isDarkMode ? 'bg-slate-900' : 'bg-slate-100'}`}>
                            {MODELS.map(m => (
                                <button
                                    key={m.id}
                                    ...
                                    className={`relative flex-shrink-0 px-4 py-3 rounded-lg font-bold transition-all flex flex-col items-center gap-2 min-w-[100px] overflow-hidden ${selectedModel === m.id
                                        ...
                                        : (isDarkMode ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-800' : 'text-slate-500 hover:text-slate-700 hover:bg-white')
                                        }`}
                    ...
                    {/* Parameter Card */}
                    <div className={`p-6 rounded-xl border transition-all duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        ...
                        <div className={`space-y-6 border-t pt-6 ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                                ...
                                <>
                                    <div className="space-y-3">
                                        ...
                                        <input
                                            ...
                                            style={{
                                                background: `linear-gradient(to right, ${primaryStr} ${((params.knn.k - 1) / 24) * 100}%, ${isDarkMode ? '#334155' : '#e2e8f0'} ${((params.knn.k - 1) / 24) * 100}%)`,
                            ...
                            {selectedModel === 'rf' && (
                                <>
                                    ...
                                    <div className="space-y-3 mt-6">
                                        <div className="flex justify-between items-center">
                                            <Tooltip isDarkMode={isDarkMode} content="Limits how deep each tree can search. Deeper trees learn more detail, but shallow trees generalize better to new patients.">
                                                <label className={`text-sm font-medium flex items-center gap-1.5 cursor-help ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                                    Maximum Depth Per Tree <HelpCircle className="w-3.5 h-3.5 opacity-50" />
                                                </label>
                                            </Tooltip>
                                            <span className={`text-sm font-mono font-bold`} style={{ color: primaryStr }}>{params.rf.maxDepth}</span>
                                        </div>
                                        <input
                                            type="range" min="1" max="10" step="1"
                                            value={params.rf.maxDepth}
                                            onChange={(e) => setParams({ ...params, rf: { ...params.rf, maxDepth: Number(e.target.value) } })}
                                            className={`w-full h-2 rounded-lg appearance-none cursor-pointer transition-all ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-200'}`}
                                            style={{
                                                background: `linear-gradient(to right, ${primaryStr} ${((params.rf.maxDepth - 1) / 9) * 100}%, ${isDarkMode ? '#334155' : '#e2e8f0'} ${((params.rf.maxDepth - 1) / 9) * 100}%)`,
                                                accentColor: primaryStr
                                            }}
                                        />
                                        <div className="flex justify-between text-[10px] text-slate-500">
                            ...
                            {selectedModel === 'lr' && (
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                        <Tooltip isDarkMode={isDarkMode} content="How many times the algorithm updates its estimates. More iterations allow it to find a better curve, but takes longer to train.">
                                            <label className={`text-sm font-medium flex items-center gap-1.5 cursor-help ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                                Maximum Iterations <HelpCircle className="w-3.5 h-3.5 opacity-50" />
                                            </label>
                                        </Tooltip>
                                        <span className={`text-sm font-mono font-bold`} style={{ color: primaryStr }}>{params.lr.iterations}</span>
                                    </div>
                                    <input
                                        type="range" min="100" max="5000" step="100"
                                        value={params.lr.iterations}
                                        onChange={(e) => setParams({ ...params, lr: { ...params.lr, iterations: Number(e.target.value) } })}
                                        className={`w-full h-2 rounded-lg appearance-none cursor-pointer transition-all ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-200'}`}
                                        style={{
                                            background: `linear-gradient(to right, ${primaryStr} ${((params.lr.iterations - 100) / 4900) * 100}%, ${isDarkMode ? '#334155' : '#e2e8f0'} ${((params.lr.iterations - 100) / 4900) * 100}%)`,
                                            accentColor: primaryStr
                                        }}
                                    />
                                    <div className="flex justify-between text-[10px] text-slate-500">
                                        <span>100 (Quick)</span>
                                        <span>5000 (Thorough)</span>
                                    </div>
                                </div>
                            )}
                ...
                <motion.div variants={itemAnim} className="lg:col-span-8 space-y-6">
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className={`divide-y ${isDarkMode ? 'divide-slate-700 text-slate-300' : 'divide-slate-100 text-slate-700'}`}>
                                    {comparisonList.length > 0 ? comparisonList.map((res) => (
                                        <tr key={res.id} className={`${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-slate-50/80'} transition-colors`}>
                    ...
                    {comparisonList.length > 0 && (
                        <div className={`rounded-xl border overflow-hidden relative shadow-sm ${isDarkMode ? 'bg-slate-900 border-slate-700/50 ' : 'bg-white border-slate-200'}`}>
                            {/* Ambient Glow */}
                            ...
                            <div className={`px-6 py-4 border-b flex items-center justify-between relative z-10 ${isDarkMode ? 'border-slate-800 bg-slate-800' : 'border-slate-100 bg-slate-50/50'}`}>
                                            ...
                                            <div className="space-y-1.5 relative py-2">
                                                ...
                                                <div className="absolute inset-0 flex items-center gap-3 pointer-events-none px-0">
                                                    ...
                                                    <div className="flex-1 h-full relative">
                                                        <div
                                                            className={`absolute top-0 bottom-0 w-[2px] ${isDarkMode ? 'bg-red-500/50' : 'bg-red-500/40'} border-l border-slate-700`}
            ...
            <div className={`md:hidden flex justify-between items-center pt-8 border-t mt-8 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                ...
                <button
                    onClick={onNext}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold shadow-sm transition-all text-white `}
