...
import {
    ...
    CheckCircle2, AlertTriangle, Info, Lock,
...
const STEPS_TABLE = [
    { n: 1, name: 'Clinical Scenario', icon: BookOpen, what: 'Read about the medical problem the AI is trying to solve in your chosen specialty.', access: 'Always available', color: 'text-sky-500' },
    { n: 2, name: 'Dataset Upload', icon: Database, what: 'Upload a patient dataset (or use the built-in example) and review the data.', access: 'Always available', color: 'text-violet-500' },
    { n: 3, name: 'Preprocessing', icon: Sliders, what: 'Choose how to handle missing values, normalise measurements, and split data into training and test groups.', access: 'Unlocks after Step 2', color: 'text-amber-500' },
    { n: 4, name: 'Algorithm Choice', icon: Activity, what: 'Select one of six AI model types and adjust its settings using sliders.', access: 'Unlocks after Step 3', color: 'text-indigo-500' },
    { n: 5, name: 'Evaluation Metrics', icon: BarChart3, what: 'See how well the AI performed, including how many patients were correctly identified.', access: 'Unlocks after Step 4', color: 'text-emerald-500' },
    { n: 6, name: 'Prediction Insights', icon: Lightbulb, what: 'Find out which measurements mattered most and why the AI made a specific prediction for a patient.', access: 'Unlocks after Step 5', color: 'text-orange-500' },
    { n: 7, name: 'Fairness Checks', icon: Shield, what: 'Check whether the AI treats different patient groups fairly and review an EU AI Act compliance checklist.', access: 'Always available', color: 'text-rose-500' },
];
...
const SpecialtiesTab = ({ isDarkMode }) => (
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto rounded-xl bg-white border border-slate-200 shadow-md dark:border-slate-700">
            <table className="w-full text-xs">
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-slate-700' : 'divide-slate-100'}`}>
                    {SPECIALTIES.map(s => (
                        <tr key={s.n} className={`${isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-50 text-slate-700'} transition-colors`}>
...
const StepsTab = ({ isDarkMode }) => (
    <div className="space-y-6">
        <div>
            ...
            <div className="overflow-x-auto rounded-xl bg-white border border-slate-200 shadow-md dark:border-slate-700 mb-6">
                <table className="w-full text-xs">
                    <thead className={isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-50 text-slate-500'}>
                        <tr>
        ...
        {[
            {
                n: 1, title: 'Step 1 — Clinical Scenario', color: 'sky',
            ...
            {
                n: 2, title: 'Step 2 — Dataset Upload', color: 'violet',
            ...
            {
                n: 3, title: 'Step 3 — Preprocessing', color: 'amber',
            ...
            {
                n: 4, title: 'Step 4 — Algorithm Choice', color: 'indigo',
            ...
            {
                n: 5, title: 'Step 5 — Evaluation Metrics', color: 'emerald',
            ...
            {
                n: 6, title: 'Step 6 — Prediction Insights', color: 'orange',
            ...
            {
                n: 7, title: 'Step 7 — Fairness Checks', color: 'rose',
            ...
            <div key={s.n} className={`p-5 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-3">
            {MODELS_TABLE.map((m, i) => (
                <div key={i} className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                    ...
                    <div className={`text-[11px] px-3 py-2 rounded-lg ${isDarkMode ? 'bg-slate-900 text-slate-400' : 'bg-slate-50 text-slate-500'}`}>
    ...
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto rounded-xl bg-white border border-slate-200 shadow-md dark:border-slate-700 mb-6">
            <table className="w-full text-xs">
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-slate-700' : 'divide-slate-100'}`}>
                    {METRICS_TABLE.map(m => (
                        <tr key={m.name} className={`${isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-50 text-slate-700'}`}>
            ...
            <table className={`w-full text-xs border rounded-xl overflow-hidden ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                ...
                <tbody>
                    <tr className={`border-t ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                        <td className={`px-4 py-3 font-semibold text-xs ${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-50'}`}>Actually NOT at Risk</td>
                    ...
                    <tr className={`border-t ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                        <td className={`px-4 py-3 font-semibold text-xs ${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-50'}`}>Actually AT RISK</td>
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                {filtered.map(g => (
                    <div key={g.term} className={`p-3.5 rounded-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className={`fixed inset-0 sm:inset-4 md:inset-8 lg:inset-12 z-[101] flex flex-col rounded-none sm:rounded-xl border shadow-sm overflow-hidden
                            ${isDarkMode ? 'bg-[#0F172A] border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                    >
                        {/* Header */}
                        <div className={`flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b shrink-0 ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-white'}`}>
                        ...
                        {/* Tab Bar */}
                        <div className={`flex gap-1 px-4 py-2 border-b overflow-x-auto shrink-0 ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-white'}`}>
                            {TABS.map(tab => {
                                const Icon = tab.icon;
                        ...
                        {/* Footer */}
                        <div className={`px-6 py-3 border-t flex items-center justify-between text-[11px] shrink-0 ${isDarkMode ? 'border-slate-700 bg-slate-900 text-slate-500' : 'border-slate-200 bg-white text-slate-400'}`}>
