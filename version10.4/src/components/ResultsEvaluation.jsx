...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const Card = ({ children, title, subtitle, action, className = '', isDarkMode, id }) => (
    <div id={id} className={`border rounded-2xl flex flex-col overflow-hidden transition-colors ${isDarkMode ? 'bg-[#131316] border-[#1f1f23]' : 'bg-white border-gray-200 shadow-sm'} ${className}`}>
        {(title || action) && (
...
/* ═══════════════════════════════════════════════════════════════
   ROC Curve Content
   ═══════════════════════════════════════════════════════════════ */
const ROCCurve = ({ auc = 0.5, sensitivity = 0, specificity = 0, isDarkMode }) => {
    // True ROC curve for binary predictions: (0,0) -> (1-specificity, sensitivity) -> (1,1)
    const points = [
        { x: 0, y: 100 }, 
        { x: (1 - specificity) * 100, y: 100 - (sensitivity * 100) },
        { x: 100, y: 0 } 
    ];
            ...
            <div className="relative z-10 px-4 md:px-6 pt-6 space-y-6">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        ═══════════════════════════════════════════ */}
                        <div id="s5-metrics" className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-6 items-stretch">
                            ...
                            <Card id="s5-roc" isDarkMode={isDarkMode} title="ROC Curve — Separability" className="lg:col-span-7">
                                <ROCCurve auc={results.auc} sensitivity={results.sensitivity} specificity={results.specificity} isDarkMode={isDarkMode} />
                            </Card>

                            {/* RIGHT CARDS (Stacked) */}
                            <div className="lg:col-span-5 flex flex-col gap-5">
                                {/* Top right: Confusion Details */}
                                <Card id="s5-confusion" isDarkMode={isDarkMode} title="Confusion Matrix Details">
                                ...
                                <Card id="s5-monitoring" isDarkMode={isDarkMode} title="Model Performance Monitoring">
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
                    ...
                    <motion.button onClick={onNext} whileHover={{ scale: 1.02, x: 3 }} whileTap={{ scale: 0.97 }}
                        className="flex items-center gap-2 px-6 py-2.5 rounded-2xl text-sm font-semibold text-white transition-all duration-300 shadow-sm focus:outline-none"
                ...
                steps={[
                    {
                        ...
                        position: 'below',
                        pose: 'wave',
                        title: 'Top-Level Evaluation Metrics',
                        body: 'These six metrics summarize how well your model performs. Accuracy is overall correctness, but we also look closely at Sensitivity (how many true positives we caught) and Specificity (how many false alarms we avoided). F1-Score balances the two.',
                        sub: 'In a clinical setting, missing a diagnosis (low sensitivity) can be fatal, while too many false alarms (low specificity) cause alarm fatigue. We must balance both.',
                    },
                    {
                        targetId: 's5-roc',
                        position: 'right',
                        pose: 'point',
                        title: 'ROC Curve & Separability',
                        body: 'The Receiver Operating Characteristic (ROC) curve visualizes the trade-off between Sensitivity (true positives) and 1-Specificity (false positives) across different thresholds. The higher the curve bows toward the top-left corner, the better the model is at separating the two classes.',
                        sub: 'The Area Under the Curve (AUC) summarizes this plot. An AUC of 1.0 is perfect, 0.5 is random guessing. It shows us if the model can truly distinguish patients.',
                    },
                    {
                        targetId: 's5-confusion',
                        position: 'left',
                        pose: 'wave',
                        title: 'Confusion Matrix Breakdown',
                        body: 'This breaks down predictions into four categories: True Negatives (Correctly Safe), True Positives (Correctly High-Risk), False Positives (False Alarms), and False Negatives (Missed Patients). You can see exactly where the model is making mistakes.',
                        sub: 'False Negatives are our worst enemy here — sending a sick patient home is dangerous. We aim to keep the red "Missed Patient" count as close to zero as possible.',
                    },
                    {
                        targetId: 's5-monitoring',
                        position: 'left',
                        pose: 'point',
                        title: 'Performance Monitoring',
                        body: 'This visualizes the simple ratio of correct vs. incorrect predictions (error rate). A high error rate indicates the model struggles to generalize to the test data. If it\'s too high, you might need to adjust hyperparameters back in Step 4.',
                        sub: 'Continuous monitoring is essential. Even a deployed model needs its "vitals" checked regularly to ensure it hasn\'t degraded over time.',
                    },
                ]}
            />
        </>
