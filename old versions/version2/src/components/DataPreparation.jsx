...
const NormalisationViz = React.memo(({ normMethod, isDarkMode, dataset, datasetSchema }) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        <AnimatedBar label="Minimum" value={2} displayValue={`${raw.min}`} color="bg-red-500" delay={0} isDarkMode={isDarkMode} />
                        <AnimatedBar label="Average" value={50} displayValue={`${raw.avg}`} color="bg-indigo-500" delay={0.15} isDarkMode={isDarkMode} />
                        <AnimatedBar label="Maximum" value={100} displayValue={`${raw.max}`} color="bg-emerald-500" delay={0.3} isDarkMode={isDarkMode} />
                    </div>
                </div>
...
const ClassBalanceViz = React.memo(({ imbalanceMethod, isDarkMode, dataset, targetColumn }) => {
...
const VisualizationsPanel = React.memo(({ isApplied, isAnimating, isDarkMode, normMethod, imbalanceMethod, dataset, datasetSchema, targetColumn }) => {
    return (
        <>
            {/* Loading Overlay */}
            <AnimatePresence>
            ...
            {!isApplied && !isAnimating ? (
                ...
                >
                    {/* Normalisation Viz */}
                    <NormalisationViz normMethod={normMethod} isDarkMode={isDarkMode} dataset={dataset} datasetSchema={datasetSchema} />

                    {/* Class Balance Viz */}
                    <ClassBalanceViz imbalanceMethod={imbalanceMethod} isDarkMode={isDarkMode} dataset={dataset} targetColumn={targetColumn} />
                </motion.div>
            ) : null}
        </>
    );
...
const DataPreparation = ({ isDarkMode, onNext, onPrev, patientCount, dataset, datasetSchema, targetColumn }) => {
