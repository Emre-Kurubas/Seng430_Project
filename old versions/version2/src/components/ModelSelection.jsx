...
const ModelSelection = ({ isDarkMode, onNext, onPrev, dataset, datasetSchema, targetColumn, setTrainedModelResult }) => {
    ...
    useEffect(() => {
        if (autoRetrain) {
            const timer = setTimeout(() => {
                trainModel();
            }, 600); // Increased debounce to allow page transition to finish
        ...
        return (
            <div className={`w-full flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in duration-500 rounded-3xl border ${isDarkMode ? 'bg-slate-800/20 border-slate-700/50' : 'bg-slate-100 border-slate-200'}`}>
                <div className="relative mb-8">
                    <div className={`w-24 h-24 border-4 rounded-full animate-spin ${isDarkMode ? 'border-slate-700 border-t-indigo-500' : 'border-slate-200 border-t-indigo-600'}`} />
                    <div className={`absolute inset-3 border-4 rounded-full animate-spin ${isDarkMode ? 'border-slate-700/50 border-b-emerald-400' : 'border-slate-200/50 border-b-emerald-500'}`} style={{ animationDirection: 'reverse', animationDuration: '0.8s' }} />
                    <Network className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'} animate-pulse`} />
                </div>
                ...
                <div className={`mt-8 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest animate-pulse ${isDarkMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-700'}`}>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        <div className={`px-5 py-3 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
                            ...
                            {isTraining && (
                                <span className={`text-xs font-mono text-emerald-500 animate-pulse px-2 py-1 rounded ${isDarkMode ? 'bg-slate-900/80' : 'bg-emerald-50'}`}>Training...</span>
                            )}
                        </div>
                        <div className="p-5">
