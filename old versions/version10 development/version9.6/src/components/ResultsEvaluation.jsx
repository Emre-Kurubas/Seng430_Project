...
const ResultsEvaluation = ({ isDarkMode, onNext, onPrev, onGoToStep, trainedModelResult, domain }) => {
    ...
    const getThresholdMetrics = (t, base) => {
        ...
        // Mathematically tie threshold slider to a synthetic ROC distribution curve
        // anchored perfectly at (t=50) to the real model's default metric output
        let sens, spec;
        if (t === 50) {
            sens = base.sensitivity;
            spec = base.specificity;
        } else if (t < 50) {
            // As threshold lowers, sensitivity increases towards 1 (flags more)
            // specificity drops towards 0 (more false alarms)
            const factor = (50 - t) / 50; 
            sens = base.sensitivity + (1 - base.sensitivity) * factor;
            spec = base.specificity * (1 - factor);
        } else {
            // As threshold rises, sensitivity drops towards 0 (misses more)
            // specificity increases towards 1 (fewer false alarms)
            const factor = (t - 50) / 50;
            sens = base.sensitivity * (1 - factor);
            spec = base.specificity + (1 - base.specificity) * factor;
        }

        // Project new precision using generic distribution assumption (P=0.5)
        const P = 0.5, N = 0.5;
        const TP = sens * P;
        const FP = (1 - spec) * N;
                    ...
                    <motion.div variants={containerAnim} initial="hidden" animate="show" className="w-full space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                ...
                                >
                                    <CMCell value={results.tp} label="Correctly Caught" sub="True Positives" type="tp" isDarkMode={isDarkMode} delay={0.1} />
                                    <CMCell value={results.fn} label="Missed Risk" sub="False Negatives" type="fn" isDarkMode={isDarkMode} delay={0.2} />
                                    <CMCell value={results.tn} label="Correctly Safe" sub="True Negatives" type="tn" isDarkMode={isDarkMode} delay={0.15} />
                                    <CMCell value={results.fp} label="False Alarms" sub="False Positives" type="fp" isDarkMode={isDarkMode} delay={0.25} />
                                </motion.div>
                            ...
                            <motion.div variants={itemAnim} className={'rounded-3xl p-6 flex flex-col transition-all duration-300 ' + (isDarkMode
                                ...
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <div className={'flex items-center gap-1.5 text-[10px] font-bold '
                                        + (results.auc >= 0.80 ? (isDarkMode ? 'text-emerald-400' : 'text-emerald-600')
                                        : results.auc >= 0.60 ? (isDarkMode ? 'text-amber-400' : 'text-amber-600')
                                        : (isDarkMode ? 'text-red-400' : 'text-red-600'))}>
                                        <CheckCircle2 className="w-3 h-3" />
                                        AUC {Number(results.auc).toFixed(2)}
                                    </div>
                                </div>
                                <p className={'text-[11px] mb-4 ' + (isDarkMode ? 'text-slate-500' : 'text-slate-400')}>
                                    {results.auc >= 0.90 ? 'Excellent' : results.auc >= 0.80 ? 'Good' : results.auc >= 0.60 ? 'Acceptable' : 'Poor'} discriminative ability. The diagonal = random guessing.
                                </p>
