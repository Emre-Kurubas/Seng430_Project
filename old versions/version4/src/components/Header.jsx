...
/**
 * @component Header
 * @description Sticky top header bar with logo, active domain pill, reset, theme toggle, and help button.
 * @param {Object} props
 * @param {Object} props.activeDomain - Currently selected medical specialty
 * @param {boolean} props.isDarkMode - Whether dark mode is active
 * @param {Function} props.toggleTheme - Callback to switch light/dark mode
 * @param {Function} props.onHelpOpen - Callback to open the help modal
 * @param {Function} props.onReset - Callback for the two-click reset flow
 * @param {boolean} props.resetPending - Whether the first reset click has been registered
 */
const Header = ({ activeDomain, isDarkMode, toggleTheme, onHelpOpen, onReset, resetPending }) => {
    const primaryStr = activeDomain?.theme?.primary || '#6366f1';
    const secondaryStr = activeDomain?.theme?.secondary || '#10b981';
            ...
            `}>
                ...
                <div className="flex items-center gap-3">
                    <motion.div
                        ...
                        style={{ background: `linear-gradient(135deg, ${secondaryStr}, ${primaryStr})`, boxShadow: `0 4px 16px ${primaryStr}59` }}
                    >
                    ...
                    <div>
                        <h1 className={`font-black text-[15px] sm:text-[17px] leading-none tracking-[-0.02em] flex items-center gap-1.5 sm:gap-2 font-[Outfit] ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                            ...
                            <span className={`hidden sm:inline px-2 py-0.5 rounded-full text-[9px] font-bold tracking-[0.12em] uppercase border`} style={{ backgroundColor: `${primaryStr}26`, color: primaryStr, borderColor: `${primaryStr}40` }}>
                                ML TOOL
                            </span>
                        </h1>
                ...
                <div className="flex items-center gap-1 sm:gap-1.5">
                        ...
                        {activeDomain && (
                            ...
                            >
                                <span className="relative flex h-1.5 w-1.5">
                                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-60`} style={{ backgroundColor: secondaryStr }} />
                                    <span className={`relative inline-flex rounded-full h-1.5 w-1.5`} style={{ backgroundColor: secondaryStr }} />
                                </span>
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.9 }}
                        onClick={onReset}
                        className={`relative flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-xl text-[11px] font-semibold border transition-all duration-200
                            ${resetPending
                                ? 'border-red-500/60 text-red-400 bg-red-500/10 animate-pulse shadow-[0_0_12px_rgba(239,68,68,0.2)]'
                                : isDarkMode
                                    ? 'border-slate-700/60 text-slate-400 hover:text-slate-200 hover:border-slate-600 hover:bg-slate-800/60'
                                    : 'border-slate-200 text-slate-500 hover:text-slate-700 hover:bg-slate-50'
                            }`}
                        title={resetPending ? 'Click again to confirm reset' : 'Reset progress'}
                        aria-label={resetPending ? 'Click again to confirm reset' : 'Reset progress'}
                    >
                        <motion.div animate={{ rotate: resetHover || resetPending ? 360 : 0 }} transition={{ duration: 0.4 }}>
                    ...
                    <motion.button
                        ...
                        style={{ background: `linear-gradient(135deg, ${primaryStr} 0%, ${secondaryStr} 100%)`, boxShadow: `0 4px 16px ${primaryStr}59` }}
