...
/** @description Scroll arrow for the step strip */
const ArrowBtn = ({ onClick, dir, isDarkMode }) => (
    <button
        onClick={onClick}
        type="button"
        aria-label={dir === 'left' ? 'Scroll steps left' : 'Scroll steps right'}
...
/**
 * @component Stepper
 * @description Horizontal 7-step pipeline navigation bar with progress indicator, locking logic, and scroll support.
 * @param {Object} props
 * @param {number} props.currentStep - Current active step (1–7)
 * @param {boolean} props.isDarkMode - Dark mode flag
 * @param {number} props.maxUnlockedStep - Highest step the user has reached
 * @param {Function} props.onStepClick - Callback to navigate to a specific step
 * @param {Object} props.domain - Selected medical domain (for theme colours)
 */
const Stepper = ({ currentStep = 1, isDarkMode, maxUnlockedStep = 1, onStepClick, domain }) => {
    ...
    return (
        <div id="stepper-bar" className={`rounded-2xl border overflow-hidden transition-all duration-500 ${isDarkMode ? 'bg-[#0C1428]/75 border-white/[0.06] shadow-[0_8px_32px_rgba(0,0,0,0.3)] backdrop-blur-2xl' : 'bg-white/75 border-slate-200/80 shadow-lg backdrop-blur-2xl'}`}>
            {/* Top progress bar */}
            <div className={`h-[3px] ${isDarkMode ? 'bg-white/5' : 'bg-slate-100'}`} role="progressbar" aria-valuenow={currentStep} aria-valuemin={1} aria-valuemax={7} aria-label={`Pipeline progress: step ${currentStep} of 7`}>
                <motion.div
                    className="h-full rounded-full progress-glow"
                    style={{ background: `linear-gradient(90deg, ${secondaryStr}, ${primaryStr})` }}
                        ...
                        return (
                            <React.Fragment key={step.id}>
                                ...
                                <motion.div
                                    ...
                                    aria-label={`Step ${step.id}: ${step.title}${isActive ? ' (current)' : isCompleted ? ' (completed)' : isLocked ? ' (locked)' : ''}`}
                                    className={`
                                ...
                                >
                                    ...
                                    {isActive && (
                                        ...
                                        layoutId="stepper-active"
                                            ...
                                            style={{ boxShadow: `0 0 0 1.5px ${primaryStr}50, 0 0 16px ${primaryStr}20` }}
                                    ...
                                    <div
                                        ...
                                        style={
                                            isActive
                                                ? { background: `linear-gradient(135deg, ${primaryStr}dd, ${primaryStr}88)`, boxShadow: `0 2px 8px ${primaryStr}40` }
                                                : isCompleted && isUnlocked
