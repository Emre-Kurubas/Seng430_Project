  ...
  return (
    <Float speed={1.5} rotationIntensity={1.5} floatIntensity={2}>
      <mesh ref={sphereRef} scale={1.8}>
        ...
        <MeshDistortMaterial
          color={isDarkMode ? "#4f46e5" : "#10b981"}
          attach="material"
          distort={0.4}
          speed={1.5}
          roughness={0.2}
...
function SmallNodes({ isDarkMode }) {
  ...
  // L5: fixed — use deterministic positions based on index, not Math.random()
  const nodes = useMemo(() => {
    return [
      { position: [-2.8,  1.5,  1.2] },
      { position: [ 3.1, -2.0, -1.8] },
      { position: [-1.2, -3.1,  2.4] },
      { position: [ 2.4,  3.2, -0.8] },
      { position: [-3.6, -1.0,  0.6] },
    ];
  }, []);
...
// H2: frameloop="demand" means Three.js only re-renders when state/props change,
// not at a constant 60fps — massive GPU/CPU savings when the element is static.
