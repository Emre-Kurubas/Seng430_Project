...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const ModelCard = ({ model, isSelected, isDarkMode, onClick, primaryStr }) => {
    ...
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className={'relative group rounded-2xl p-4 transition-all duration-300 text-left overflow-hidden w-full ' + (isSelected
                ? 'ring-2 shadow-lg'
                : (isDarkMode
                    ? 'bg-white/[0.02] border border-white/[0.06] hover:border-white/[0.12]'
                    : `bg-indigo-50/40 border border-indigo-100 shadow-sm ${isSelected ? '' : 'opacity-80 hover:opacity-100'}`)
            )}
            style={isSelected ? {
                backgroundColor: isDarkMode ? `${model.color}12` : `${model.color}15`,
        ...
        >
            {/* Hover glow */}
            <div
                className="absolute -top-8 -right-8 w-20 h-20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-2xl pointer-events-none"
                style={{ background: model.color }}
            />
            <div className="relative z-10 flex items-center gap-3">
                <div
                    className={'w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 ' + (isSelected ? '' : (isDarkMode ? 'bg-white/[0.05]' : 'bg-slate-50'))}
                    style={isSelected ? { backgroundColor: `${model.color}20` } : {}}
                >
                    <Icon className="w-5 h-5 transition-colors" style={{ color: isSelected ? model.color : (isDarkMode ? '#64748b' : '#94a3b8') }} />
                </div>
                <div className="min-w-0 flex-1">
                    <div className={'text-sm font-semibold truncate transition-colors ' + (isSelected
                        ? (isDarkMode ? 'text-white' : 'text-slate-900')
                        : (isDarkMode ? 'text-slate-300' : 'text-slate-600')
                    )}>{model.shortName}</div>
                    <div className={'text-[10px] truncate transition-colors ' + (isDarkMode ? 'text-slate-500' : 'text-slate-400')}>
                        {model.name}
                    </div>
                </div>
                {isSelected && (
                    ...
                    >
                        <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: model.color }} />
                    </motion.div>
                )}
            </div>
            {/* Bottom accent line */}
...
const ParamSlider = ({ label, tooltip, value, onChange, min, max, step, lowLabel, highLabel, isDarkMode, primaryStr, displayValue }) => (
    <div className="space-y-3">
        ...
        <input
            ...
            value={value}
            onChange={onChange}
            className={'w-full h-1.5 rounded-lg appearance-none cursor-pointer ' + (isDarkMode ? 'bg-slate-700/50' : 'bg-slate-200')}
            style={{
...
const ModelSelection = ({ isDarkMode, onNext, onPrev, dataset, datasetSchema, targetColumn, setTrainedModelResult, domain, comparisonList, setComparisonList }) => {
    ...
    if (isInitialLoading) {
        return (
            <div className={`relative w-full flex flex-col items-center justify-center min-h-[60vh] rounded-3xl border shadow-md overflow-hidden ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-blue-50/40 border-blue-100'}`}>
        ...
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="relative w-full pb-20">
            ...
            <div className="relative z-10 space-y-8">
                ...
                <motion.div variants={itemAnim} className="mb-2">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            <motion.div
                                ...
                                className={'w-10 h-10 rounded-2xl flex items-center justify-center ' + (isDarkMode ? 'bg-white/[0.05]' : 'bg-violet-100/80')}
                                style={{ boxShadow: `0 0 20px ${primaryStr}15` }}
                            >
                                <Cpu className="w-5 h-5" style={{ color: primaryStr }} />
                            </motion.div>
                    ...
                    >
                        Pick an algorithm, tweak its parameters, and let it learn from the training patients. Compare results across models.
                    </motion.p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                    ...
                    <motion.div variants={itemAnim} className="lg:col-span-4 space-y-5">
                        ...
                        <motion.div
                            ...
                            className={'rounded-3xl p-6 transition-all duration-300 relative overflow-hidden ' + (isDarkMode
                                ? 'bg-white/[0.02] border border-white/[0.06]'
                                : 'bg-indigo-50/50 border border-indigo-100 shadow-md')}
                        >
                            {/* Header glow */}
                            <div className="absolute -top-12 -right-12 w-32 h-32 rounded-full blur-3xl pointer-events-none opacity-30" style={{ background: currentModelData.color }} />
                            ...
                            <div className="relative z-10">
                                ...
                                <div className={'pt-5 border-t space-y-6 ' + (isDarkMode ? 'border-slate-800' : 'border-slate-100')}>
                                    <div className="flex items-center gap-2">
                                        <Settings className="w-3.5 h-3.5" style={{ color: currentModelData.color }} />
                                        <span className={'text-[10px] font-bold uppercase tracking-widest ' + (isDarkMode ? 'text-slate-500' : 'text-slate-400')}>Parameters</span>
                                    </div>
                                ...
                                <div className="flex flex-col gap-3 mt-6">
                                    <div className="flex gap-3">
                                        ...
                                        <motion.button
                                            ...
                                            className={'flex-1 flex gap-2 justify-center items-center py-3 rounded-2xl font-bold text-sm border-2 transition-all duration-300 ' + (
                                                (!lastResult || isTraining || isAlreadyCompared)
                                                    ? (isDarkMode ? 'border-slate-800 text-slate-600 cursor-not-allowed' : 'border-slate-200 text-slate-300 cursor-not-allowed')
                                                    : ''
                                            )}
                    ...
                    <motion.div variants={itemAnim} className="lg:col-span-8 space-y-5">
                        ...
                        <div className={'rounded-3xl overflow-hidden transition-all duration-300 ' + (isDarkMode
                            ? 'bg-white/[0.02] border border-white/[0.06]'
                            : 'bg-teal-50/50 border border-teal-100 shadow-md')}>
                            <div className={'px-6 py-4 border-b flex items-center justify-between ' + (isDarkMode ? 'border-slate-800' : 'border-slate-100')}>
                                <div className="flex items-center gap-2">
                            ...
                            {comparisonList.length > 0 && (
                                <motion.div
                                    ...
                                    className={'rounded-3xl overflow-hidden relative transition-all duration-300 ' + (isDarkMode
                                        ? 'bg-white/[0.02] border border-white/[0.06]'
                                        : 'bg-blue-50/40 border border-blue-100 shadow-md')}
                                >
                                    {/* Ambient Glow */}
