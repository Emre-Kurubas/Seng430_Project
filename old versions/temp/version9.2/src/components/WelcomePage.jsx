...
const WelcomePage = ({ isDarkMode, onGetStarted, toggleTheme }) => {
  ...
  // Smooth slow scroll for intro sequence
  const handleSlowScroll = (e) => {
    e.preventDefault();
    const duration = 2500; // 2.5 seconds to smoothly scroll past intro
    const start = window.scrollY;
    const targetNode = document.getElementById('how-it-works');
    const target = targetNode ? targetNode.getBoundingClientRect().top + start - 40 : start + window.innerHeight * 1.2;
    let startTime = null;
        ...
        >
          ...
          <motion.div {...fadeUp(0.45)} className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <button
              ...
              className={`flex items-center gap-2 text-sm font-medium transition-colors cursor-pointer bg-transparent border-none ${
