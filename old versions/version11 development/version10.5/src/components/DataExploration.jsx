    ...
    return (
        <motion.div variants={containerAnim} initial="hidden" animate="show" className="relative max-w-7xl mx-auto w-full">
            ...
            <div className="relative z-10 space-y-6">
                ...
                <AnimatePresence>
                    {isManageDataOpen && (
                        ...
                        >
                            <div id="s2-data-source" className={`p-6 rounded-[32px] overflow-hidden relative ${isDarkMode ? 'bg-white/[0.02] border border-white/[0.06]' : 'bg-white border border-slate-200 shadow-sm'}`}>
                ...
                {/* Top Metrics Grid */}
                <motion.div variants={itemAnim} id="s2-metrics-grid" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <DashboardStatCard
                ...
                <motion.div variants={itemAnim} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div id="s2-flow-chart" className={`lg:col-span-2 p-6 rounded-[32px] overflow-hidden relative ${isDarkMode ? 'bg-white/[0.02] border border-white/[0.06]' : 'bg-white border border-slate-200 shadow-sm'}`}>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div id="s2-class-distribution" className={`flex-1 p-6 rounded-[32px] overflow-hidden relative ${isDarkMode ? 'bg-white/[0.02] border border-white/[0.06]' : 'bg-white border border-slate-200 shadow-sm'}`}>
                        ...
                        {/* Data Quality Gauge */}
                        <div id="s2-gauge" className={`p-6 py-8 rounded-[32px] overflow-hidden relative ${isDarkMode ? 'bg-white/[0.02] border border-white/[0.06]' : 'bg-white border border-slate-200 shadow-sm'}`}>
                            <GaugeChart 
                                value={100 - parseFloat(stats.missing || '0')} 
                ...
                <motion.div variants={itemAnim} className={`flex justify-between items-center pt-8 mt-8 border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-2xl text-sm font-semibold transition-all duration-300 ${isMapped ? 'text-white' : isDarkMode ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400'}`}
                        style={isMapped ? { backgroundColor: secondaryStr } : {}}
                    >
            ...
            <StepTour
                ...
                steps={[
                    {
                        targetId: 's2-data-source',
                        position: 'below',
                        pose: 'point',
                        title: 'Data Source Configuration',
                        body: 'This panel lets you choose your clinical dataset. The default option loads a curated, specialty-specific dataset. If you have your own patient data in CSV format, you can upload it here instead. You can also select the target outcome variable — the column your ML model will learn to predict.',
                        sub: 'In clinical research, the quality and provenance of your data is everything. Always know where your data comes from.',
                    },
                    {
                        targetId: 's2-metrics-grid',
                        position: 'below',
                        pose: 'wave',
                        title: 'Key Dataset Metrics',
                        body: 'These three cards give you an immediate clinical overview: Patient Records shows the total sample size, Available Features shows how many clinical variables are recorded per patient, and Data Completeness indicates what percentage of data points are present versus missing.',
                        sub: 'Think of these as vital signs for your dataset. A healthy dataset has a large sample size, sufficient features, and high completeness.',
                    },
                    {
                        targetId: 's2-flow-chart',
                        position: 'right',
                        pose: 'point',
                        title: 'Dataset Composition Flow',
                        body: 'This Sankey-style diagram visualizes how your raw data flows from total records into valid data and missing gaps. The thickness of each stream represents volume. It helps you see at a glance how much usable data you have for model training.',
                        sub: 'Missing data is like incomplete patient charts — it needs to be addressed before we can make reliable diagnoses.',
                    },
                    {
                        targetId: 's2-class-distribution',
                        position: 'left',
                        pose: 'wave',
                        title: 'Target Class Distribution',
                        body: 'This pyramid chart shows how your target outcome classes are distributed. In medical ML, class imbalance is very common — for example, far more patients may be healthy than diseased. Severe imbalance can bias your model toward the majority class.',
                        sub: 'If this distribution is heavily skewed, we\'ll address it in Step 3 using SMOTE oversampling. Watch for it!',
                    },
                    {
                        targetId: 's2-gauge',
                        position: 'left',
                        ...
                        body: 'This gauge represents your overall data quality. A score near 100% means very few missing values — the data is clean and ready for analysis. Lower scores indicate that imputation (filling in missing values) will be needed in the preparation phase.',
                        sub: 'In my clinical experience, datasets above 95% completeness produce the most reliable models. Below 90%, proceed with caution.',
                    },
                ]}
            />
        </motion.div>
