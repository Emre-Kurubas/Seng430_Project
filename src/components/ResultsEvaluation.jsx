...
// ─── Metric Card ──────────────────────────────────────────────────────────────
const MetricCard = ({ label, value, desc, concern, isDarkMode, star, delay = 0 }) => {
    ...
    return (
        ...
        >
            ...
            <div className={`text-2xl font-black mb-1 ${color}`}>{pct}%</div>
...
const CMCell = ({ value, label, sub, type, isDarkMode }) => {
    const styles = {
...
const InfoRow = ({ measure, meaning, concern, isDarkMode, value }) => {
...
const ResultsEvaluation = ({ isDarkMode, onNext, onPrev }) => {
    ...
    const thresholdMetrics = getThresholdMetrics(threshold, results);
            ...
            {results && (
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {/* ROC Curve */}
                        <div className={`p-6 rounded-2xl border flex flex-col ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                            <div className="flex items-center gap-2 mb-1">
