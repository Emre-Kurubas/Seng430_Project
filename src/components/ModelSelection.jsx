        ...
        <div className="w-full space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            <div className={`p-4 sm:p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div className="flex items-center justify-between w-full md:w-auto">
                        ...
                        <div className="flex items-center gap-2 md:hidden">
                            ...
                            <button
                                ...
                                className={`w-10 h-5 rounded-full transition-colors relative ${autoRetrain ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                            >
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        <button
                            onClick={() => setAutoRetrain(!autoRetrain)}
                            className={`w-11 h-6 rounded-full transition-colors relative ${autoRetrain ? 'bg-indigo-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                            ...
                            <div className="flex gap-3 pt-6">
                                ...
                                <button
                                    ...
                                    className={`flex-1 flex gap-2 justify-center items-center py-2.5 rounded-lg font-bold border transition-all duration-300 ${(!lastResult || isTraining || comparisonList.some(r => r.id === lastResult?.id))
                                        ? 'border-slate-200 text-slate-300 dark:border-slate-700/50 dark:text-slate-600 cursor-not-allowed bg-transparent'
                                        : isDarkMode
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {/* Visualization Card */}
                    <div className={`rounded-xl border min-h-[280px] sm:min-h-[400px] relative overflow-hidden flex flex-col ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        <div className={`p-4 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
                            <h3 className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                                {selectedModel === 'knn' && 'KNN Visualisation — How it Thinks'}
                                {selectedModel === 'svm' && 'SVM — Decision Boundary & Support Vectors'}
                                {selectedModel === 'dt' && 'Decision Tree — Flowchart'}
                                {selectedModel === 'rf' && 'Random Forest — Voting'}
                                {selectedModel === 'lr' && 'Logistic Regression — Probability Curve'}
                                {selectedModel === 'nb' && 'Naive Bayes — Distributions'}
                            </h3>
                            {isTraining && <span className="text-xs font-mono text-emerald-500 animate-pulse">Training...</span>}
                        </div>

                        <div className="relative flex-grow flex items-center justify-center p-6 bg-slate-50/50 dark:bg-slate-900/50 overflow-hidden">
                            <div className="relative w-full h-full max-w-lg aspect-[4/3] sm:aspect-auto">
                        ...
                        <div className={`p-4 border-t text-xs flex gap-3 items-start relative overflow-hidden ${isDarkMode ? 'bg-sky-900/10 border-slate-700/50 text-sky-200' : 'bg-sky-50 border-slate-100 text-sky-800'}`}>
                            {/* Decorative background glow */}
                            <div className="absolute top-0 right-0 w-32 h-32 bg-sky-500/10 blur-2xl rounded-full -translate-y-1/2 translate-x-1/2" />

                            <CheckCircle2 className="w-5 h-5 mt-0.5 shrink-0 text-sky-500 drop-shadow-[0_0_8px_rgba(14,165,233,0.5)]" />
                            <div className="relative z-10 space-y-1">
                                <span className="font-bold uppercase tracking-wider opacity-80 block text-[10px]">Clinical Meaning</span>
                                <span className="leading-relaxed block text-sm">
                                    {selectedModel === 'knn' && 'The model identifies patients with similar profiles. If 4/5 neighbours had heart failure, this patient likely will too.'}
                                    {selectedModel === 'svm' && 'The boundary represents the line of uncertainty. Patients far from the line are easy cases; those near are complex.'}
                                    {selectedModel === 'lr' && 'The curve shows risk probability. Steep curves mean small changes in health cause large jumps in risk.'}
                                    {selectedModel === 'dt' && 'Transparent logic flow. E.g. "Createine > 1.5 AND Age > 70" → High Risk.'}
                                    {selectedModel === 'rf' && 'Stability through consensus. 100 trees vote, making the prediction robust against outliers.'}
                                    {selectedModel === 'nb' && 'Based on pure statistics of the dataset. "Given Age=70, there is 80% chance of readmission".'}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            {/* MobileNav */}
            <div className="md:hidden flex justify-between items-center pt-8 border-t border-slate-200 dark:border-slate-800 mt-8">
                <button
                    onClick={onPrev}
