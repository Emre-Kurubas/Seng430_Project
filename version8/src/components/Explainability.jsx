...
const HBar = React.memo(({ label, clinical, importance, maxVal, isDarkMode, delay, primaryStr }) => {
    ...
    return (
        <motion.div
            ...
            className={'group relative p-3 rounded-2xl transition-all duration-300 ' + (isDarkMode ? 'bg-white/[0.02] hover:bg-white/[0.04]' : 'bg-white hover:bg-slate-50 border border-slate-100')}
        >
            <div className="flex items-center justify-between mb-2 gap-2">
                <div className="flex flex-col min-w-0">
                    <span className={'text-sm font-bold truncate ' + (isDarkMode ? 'text-slate-200' : 'text-slate-800')}>{label}</span>
                    <span className={'text-[10px] ' + (isDarkMode ? 'text-slate-500' : 'text-slate-400')}>{clinical}</span>
                </div>
                <div className="flex flex-col items-end">
                    <span className="text-xs font-mono font-black tabular-nums" style={{ color: primaryStr }}>{importance.toFixed(2)}</span>
                    <span className={'text-[8px] uppercase font-bold tracking-widest ' + (isDarkMode ? 'text-slate-600' : 'text-slate-400')}>Score</span>
                </div>
            </div>
            <div className={'h-2 rounded-full overflow-hidden ' + (isDarkMode ? 'bg-slate-800' : 'bg-slate-100')}>
                <motion.div
                    ...
                    style={{ filter: `drop-shadow(0 0 4px ${primaryStr}60)` }}
                />
            </div>
        </motion.div>
    );
...
const Explainability = ({ isDarkMode, onNext, onPrev, domain, dataset, datasetSchema, targetColumn }) => {
    ...
    const activeFeatures = React.useMemo(() => {
        if (datasetSchema && datasetSchema.length > 0) {
            return datasetSchema.filter(col => col.role === 'Number (measurement)' || col.role === 'Category').map(col => ({
                id: col.name, label: toReadable(col.name), clinical: CLINICAL_DESC[col.name.toLowerCase()] || (col.role === 'Category' ? 'Categorical feature' : 'Numeric measurement')
            }));
        }
    ...
    const whatIf = useMemo(() => {
        if (!top1) return '';
        return `If the patient's ${top1.label.toLowerCase()} improved to a more stable clinical range, the model predicts the risk would likely decrease. This demonstrates how targeted interventions on primary risk drivers can alter trajectory.`;
    }, [top1]);
            ...
            <div className="relative z-10 space-y-8">
                ...
                <motion.div variants={itemAnim} className="mb-2">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <BrainCircuit className="w-5 h-5" style={{ color: primaryStr }} />
                            </motion.div>
                            <span className={`text-[10px] tracking-[0.15em] font-bold uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Step 6 · Explain</span>
                        </div>
                    ...
                    <motion.h2 className={'text-4xl sm:text-5xl font-extrabold tracking-tight ' + (isDarkMode ? 'text-white' : 'text-slate-900')} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }}>
                        Why This <span style={{ background: `linear-gradient(135deg, ${primaryStr}, ${secondaryStr})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Prediction?</span>
                    </motion.h2>
                    <motion.p className={'text-sm mt-3 max-w-xl leading-relaxed ' + (isDarkMode ? 'text-slate-400' : 'text-slate-600')} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
                        See which measurements mattered most overall, and trace exactly why the model flagged a specific patient.
                    </motion.p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-6">
                        ...
                        <motion.div variants={itemAnim} className={'rounded-[2rem] p-6 lg:p-8 transition-all duration-300 flex flex-col backdrop-blur-xl shadow-lg relative overflow-hidden ' + (isDarkMode ? 'bg-slate-800/60 border border-slate-700' : 'bg-indigo-50/40 border border-indigo-100')}>
                            <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] pointer-events-none opacity-20" style={{ background: primaryStr }} />
                            ...
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <div>
                                    <h3 className={'text-[11px] font-bold uppercase tracking-widest leading-none ' + (isDarkMode ? 'text-slate-300' : 'text-indigo-900')}>Global Importance</h3>
                                    <p className={'text-[9px] mt-1 ' + (isDarkMode ? 'text-slate-500' : 'text-indigo-600/70')}>Ranked across all patients</p>
                                </div>
                            </div>
                            ...
                            {loadingFeatures ? (
                                ...
                                <div className="space-y-3 relative z-10 flex-1">
                                    {featureImportance.map((f, i) => (
                                        <HBar key={f.id} label={f.label} clinical={f.clinical} importance={f.importance} maxVal={maxImportance} isDarkMode={isDarkMode} delay={i * 0.07} primaryStr={primaryStr} />
                                    ))}
                                </div>
                            )}
                            ...
                            {!loadingFeatures && top1 && (
                                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
                                    className={'mt-6 p-4 rounded-2xl flex items-start gap-3 text-[11px] leading-relaxed shadow-sm ' + (isDarkMode ? 'bg-emerald-900/20 border border-emerald-500/20 text-emerald-300' : 'bg-white border border-emerald-100 text-emerald-800')}
                                >
                                    <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" style={{ color: secondaryStr }} />
                                    <div><span className="font-bold">Clinical sense-check: </span>{senseCheck}</div>
                                </motion.div>
                            )}
                        </motion.div>

                        {/* Single Patient Explain Trigger Card */}
                        <motion.div variants={itemAnim} className={'rounded-[2rem] p-6 lg:p-8 transition-all duration-300 backdrop-blur-xl shadow-lg ' + (isDarkMode ? 'bg-slate-800/60 border border-slate-700' : 'bg-blue-50/40 border border-blue-100')}>
                            <div className="flex items-center gap-2 mb-6">
                                <div className="p-2.5 rounded-xl border bg-white/10 shadow-sm" style={{ backdropFilter: 'blur(10px)', borderColor: `${primaryStr}20` }}>
                                    <User className="w-4 h-4" style={{ color: primaryStr }} />
                                </div>
                                <h3 className={'text-[11px] font-bold uppercase tracking-widest leading-none ' + (isDarkMode ? 'text-slate-300' : 'text-blue-900')}>Individual Case</h3>
                            </div>
                            
                            <label className={'block text-[11px] font-bold tracking-widest uppercase mb-3 ' + (isDarkMode ? 'text-slate-400' : 'text-blue-600/80')}>
                                1. Select Patient
                            </label>
                            <div className="relative mb-5">
                                <select
                                    ...
                                    className={'w-full appearance-none pl-4 pr-10 py-3.5 rounded-2xl border text-sm font-semibold outline-none transition-all shadow-sm focus:ring-2 ' 
                                        + (isDarkMode ? 'bg-slate-900/50 border-slate-700 text-slate-200 focus:ring-indigo-500/50' : 'bg-white border-blue-200 text-blue-900 hover:border-blue-300 focus:ring-indigo-500/30')}
                                >
                                    {patients.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
                                </select>
                                <ChevronDown className={'w-4 h-4 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none ' + (isDarkMode ? 'text-slate-500' : 'text-blue-400')} />
                            </div>
                            ...
                            <motion.button
                                ...
                                className={'w-full py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 text-white shadow-lg ' + (explaining || loadingFeatures ? 'bg-slate-700 text-slate-400 cursor-wait opacity-50 shadow-none' : '')}
                    ...
                    <motion.div variants={itemAnim} className={'lg:col-span-7 h-full flex flex-col rounded-[2rem] p-6 lg:p-8 transition-all duration-300 backdrop-blur-xl shadow-lg relative overflow-hidden ' + (isDarkMode ? 'bg-slate-800/60 border border-slate-700' : 'bg-white border border-slate-200')}>
                        <div className="absolute top-0 right-0 w-full h-[500px] rounded-full blur-[120px] pointer-events-none opacity-5" style={{ background: primaryStr }} />

                        <div className="flex items-center justify-between mb-8 relative z-10">
                            <div className="flex items-center gap-2">
                                <div className="p-2.5 rounded-xl border bg-slate-50 shadow-sm" style={{ backdropFilter: 'blur(10px)', borderColor: isDarkMode ? '#334155' : '#e2e8f0', background: isDarkMode ? '#1e293b' : '#f8fafc' }}>
                                    <HelpCircle className="w-4 h-4" style={{ color: primaryStr }} />
                                </div>
                                <div>
                                    <h3 className={'text-[11px] font-bold uppercase tracking-widest leading-none ' + (isDarkMode ? 'text-slate-300' : 'text-slate-600')}>Local Explainability</h3>
                                    <p className={'text-[9px] mt-1 ' + (isDarkMode ? 'text-slate-500' : 'text-slate-400')}>Waterfall Chart Analysis</p>
                                </div>
                            </div>
                            ...
                            {explained && patientData && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col relative z-10 space-y-4">
                                    ...
                                    <div className="space-y-1.5 pt-2">
                                        {(() => {
                                            ...
                                            return patientData.contributions.map((c, i) => (
                                                <WaterfallBar key={c.id} label={c.label} value={c.value} contribution={c.contribution} isRisk={c.isRisk} maxAbs={maxAbs} isDarkMode={isDarkMode} delay={i * 0.08} />
                                            ));
                                        })()}
                                    </div>
                                    ...
                                    <div className="pt-4 mt-auto">
                                        <div className={'p-5 rounded-2xl border flex items-start gap-4 text-xs leading-relaxed ' + (isDarkMode ? 'bg-blue-500/10 border-blue-500/20 text-blue-200' : 'bg-blue-50/80 border-blue-100 text-blue-900')}>
                                            <div className="p-2 rounded-xl bg-blue-500/20 shrink-0"><Lightbulb className="w-4 h-4 text-blue-500" /></div>
                                            <div>
                                                <span className="font-bold uppercase tracking-wider text-[10px] opacity-70 block mb-1">What-If Intervention</span>
                                                {whatIf}
                                            </div>
                                        </div>
                                    </div>
